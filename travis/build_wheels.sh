...
#/travis/build_tools.sh

# Build static 3rdparty dependencies
# (already built in mtgupf/essentia-builds images)
# /home/macklemore/essentia/packaging/build_3rdparty_static_debian.sh --with-gaia

# Location of the dependencies in essentia-builds docker images
# ...is already set in the docker images
PKG_CONFIG_PATH=/usr/local/lib/pkgconfig
PKG_CONFIG_PATH=/home/macklemore/essentia/packaging/debian_3rdparty/lib/pkgconfig
...
#     "python3" waf configure --with-gaia --with-tensorflow --build-static --static-dependencies  --with-vamp --with-python --with-examples --pkg-config-path="${PKG_CONFIG_PATH}"
# else
#     PROJECT_NAME='essentia'
#     "python3" waf configure --with-gaia --build-static --static-dependencies  --with-vamp --with-python --with-examples --pkg-config-path="${PKG_CONFIG_PATH}"
# fi

cd ../

#PROJECT_NAME='essentia-tensorflow'
# sudo python3 waf configure  --with-static-examples --with-gaia --with-tensorflow
# sudo "python3" waf install -p

PROJECT_NAME='essentia'
sudo python3 waf configure  --with-static-examples --with-gaia 
sudo "python3" waf install -p
#cd -
...
for PYBIN in /opt/python/cp3*/bin; do
    ...
    ESSENTIA_PROJECT_NAME="${PROJECT_NAME}" "pip" wheel --wheel-dir=wheelhouse/ ${PROJECT_NAME}
...
for PYBIN in /opt/python/cp3*/bin/; do
    ...
    # Test that essentia can be imported along with TensorFlow
        ...
        (cd "$HOME"; python3 -c 'import essentia; import essentia.standard; import essentia.streaming; import tensorflow')
    else
        (cd "$HOME"; python3 -c 'import essentia; import essentia.standard; import essentia.streaming')
    fi
done
